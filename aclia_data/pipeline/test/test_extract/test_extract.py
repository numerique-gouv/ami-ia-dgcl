...
class ExtractTest(unittest.TestCase):
    ...
    def test_text_from_pdf_text_with_incorrect_path(self):
        self.assertIs(
            extract.text_from_pdf_text(os.path.join(self.data_dir, "NotAValidPath.PDF")), None
        )
    ...
    def test_text_from_pdf_image_with_incorrect_path(self):
        self.assertIs(
            extract.text_from_pdf_image(os.path.join(self.data_dir, "NotAValidPath.PDF")), None
        )

    def test_read_pdf_with_pdf_text(self):
        self.assertIn(
            "Ceci est un test", extract.text_from_pdf(os.path.join(self.data_dir, "PDF_Text.PDF"))
        )

    def test_read_pdf_with_pdf_image(self):
        self.assertIn(
            "Ceci est un test", extract.text_from_pdf(os.path.join(self.data_dir, "PDF_Image.PDF"))
        )
