...
evaluate.py
=================================================
Module to evaluate the predictions of a NER model
"""
...
class Evaluator:
    ...
    def evaluate(self):
        """
        Evaluate predictions based on the data in the constructor
        """
...
def evaluate_score_to_custom(score):
    """
    Convert evaluator output to a custom output
    :param score: Dict from Evaluator().evaluate with the following keys :
    ['correct', 'incorrect', 'partial', 'missed',
    'spurious', 'possible', 'actual', 'precision', 'recall']
    :return: Dict with the following keys : ['precision', 'recall', 'support'] with rounded results
    """
    return {
        "precision": round(score["precision"], 3),
        "recall": round(score["recall"], 3),
        "support": score["possible"],
    }
...
def compute_metrics(true_named_entities, pred_named_entities, tags):
    """
    Compute metrics between the true_named_entities and the pred_named_entities for specified tags
    """
